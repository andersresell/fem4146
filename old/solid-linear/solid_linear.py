...
def create_mesh_box(nEx, nEy,nEz, Lx,Ly,Lz, element_type):
    ...
    il_to_ijk_loc = element_utils.il_to_ijk_loc_all[element_type]
    for k in range(nNz):
        for j in range(nNy):
            for i in range(nNx):
                x = i*dx
...
def suppress_boundary_box(nodeIDs, dof_status, dof_loc,nN):
    ...
    assert dof_loc==element_utils.DOF_U or dof_loc==element_utils.DOF_V or dof_loc == element_utils.DOF_W
    for I in nodeIDs:
        assert I<nN
...
def calc_Ke(coord_x, coord_y, coord_z, element_type, nu, E):
    ...
    arr_xi, arr_w = element_utils.get_gauss_rule(nQ)
    ...
    for i in range(nQ):
        for j in range(nQ):
            for k in range(nQ):
                ...
                dNdx,dNdy,dNdz = element_utils.calc_dNdx_dNdy_dNdz(xi0, xi1, xi2, coord_x,coord_y,coord_z, element_type)
                J = element_utils.calc_J(xi0,xi1,xi2,coord_x,coord_y,coord_z, element_type)
...
def calc_Me_consistent(coord_x,coord_y,coord_z,rho, element_type, integration_type):
    nNl = element_utils.element_type_to_nNl[element_type]
    nQ = element_utils.element_type_to_nQ[element_type]
    if integration_type==element_utils.GAUSS_QUADRATURE:
        arr_xi,arr_w = element_utils.get_gauss_rule(nQ)
    elif integration_type==element_utils.LOBOTTO_QUADRATURE:
        arr_xi,arr_w = element_utils.get_lobatto_rule(nQ)
    else: assert False
    Me = np.zeros((nNl,nNl))
    ...
    for i in range(nQ):
        for j in range(nQ):
            for k in range(nQ):
                ...
                J = element_utils.calc_J(xi0,xi1,xi2,coord_x,coord_y,coord_z,element_type)
                detJ = np.linalg.det(J)
                Ncompact = element_utils.calc_N(xi0,xi1,xi2,element_type)
                # N = np.zeros(3*nNl)
                # for il in range(nNl):
...
def calc_Me_lumped_row_sum(coord_x,coord_y,coord_z,rho, element_type, integration_type):
    ...
    for i in range(nNl):
        for j in range(nNl):
            Me_lumped[i] += Me[i,j]
    assert np.abs(Me_lumped.all())>element_utils.SMALL_VAL
    return Me_lumped
...
def calc_Re_ext_consistent_shitty(coord_x, coord_y, coord_z,element_type,xi_comp,  t_uniform):
    ...
    for j in range(nQ):
        for k in range(nQ):
            ...
            xi0 = 1.0
            N = element_utils.calc_N(xi0,xi1,xi2,element_type)
            J_surf = dz*dy/4
            for l in range(nNl):
