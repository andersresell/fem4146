...
def validate_load_func(f):
    if not callable(f):
        raise TypeError("Load must be a callable")
...
def check_load_validity(config: Config, mesh: Mesh, element_set_name, load_type, load_function):
    ...
    if config.problem_type == PROBLEM_TYPE_PLANE_STRESS:
        ...
        if geometry_type == ELEMENT_GEOMETRY_TYPE_EDGE:
            if load_type != LOAD_TYPE_PRESSURE:
                raise Exception(f"Only LOAD_TYPE_PRESSURE is valid for {problem_type_to_string[config.problem_type]} "
                                f"when the element set defines an edge.")
        else:
            assert geometry_type == ELEMENT_GEOMETRY_TYPE_FACE
    ...
    if config.problem_type == PROBLEM_TYPE_PLANE_STRESS:
        ...
        if num_output_components != 1:
            raise Exception(
                ...
                f"Use a single-valued load function for LOAD_TYPE_PRESSURE or LOAD_TYPE_BODY_FORCE.")
...
def integrate_face_load_consistent(config: Config, mesh: Mesh, load: Load, nodeIDs_l, e):
...
def integrate_edge_load_consistent(config: Config, mesh: Mesh, load: Load, nodeIDs_l, e):
    assert config.problem_type == PROBLEM_TYPE_PLANE_STRESS
    element_type = config.element_type
    h = config.h
...
def integrate_loads_consistent(config: Config, mesh: Mesh, solver_data: SolverData):
    ...
    for load in config.loads:
        ...
        for e in element_set.elementIDs:
            if geometry_type == ELEMENT_GEOMETRY_TYPE_EDGE:
                R_load = integrate_edge_load_consistent(config, mesh, load, nodeIDs_l, e)
            else:
                assert geometry_type == ELEMENT_GEOMETRY_TYPE_FACE
                R_load = integrate_face_load_consistent(config, mesh, load, nodeIDs_l, e)

            #====================================================================
            # Assemble contribution to the external force vector R_ext
            #====================================================================
